...
void UBDTPawnAttributeSet::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)
{
	...
	if(GEngine)
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Emerald, FString::Printf(TEXT("Attribut qui va être modifié : %s  | Nouvelle valeur: %f"), *Attribute.AttributeName, NewValue));
}
