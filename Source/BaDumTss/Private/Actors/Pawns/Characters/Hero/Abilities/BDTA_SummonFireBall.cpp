...
void UBDTA_SummonFireBall::SpawnFireBall(ABDTHeroCharacter* Hero, FName SocketName)
{
	FTransform MuzzleTransform = Hero->GetMesh()->GetSocketTransform(SocketName);
	const FVector Start = Hero->GetMesh()->GetSocketLocation(SocketName);
	const FVector End =  Hero->GetMesh()->GetSocketLocation(SocketName) + Hero->GetActorForwardVector();
...
void UBDTA_SummonFireBall::EventReceived(FGameplayTag EventTag, FGameplayEventData EventData)
{
	...
	if (GetOwningActorFromActorInfo()->GetLocalRole() != ROLE_Authority || EventTag != FGameplayTag::RequestGameplayTag(FName("Event.Montage.SpawnFireball.LeftHand")) && EventTag != FGameplayTag::RequestGameplayTag(FName("Event.Montage.SpawnFireball.RightHand")) )
		return;
	...
	SpawnFireBall(Hero, EventTag.GetTagName() == "Event.Montage.SpawnFireball.LeftHand" ? "LeftHandSocket" : "RightHandSocket");
}